...
module.exports = JhipsterGenerator.extend({
    ...
    initializing: {
        readConfig() {
            this.entityConfig = this.options.entityConfig;
            this.appConfig = this.getJhipsterAppConfig();
            if (!this.appConfig) {
        ...
        displayLogo() {
            this.log(`\nWelcome to the ${chalk.bold.yellow('JHipster DbHelper')} generator! ${chalk.yellow(`v${packagejs.version}\n`)}`);
        },
        validate() {
    ...
    writing() {
         ...
         */
        const getLiquibaseFile = type => `${jhipsterConstants.SERVER_MAIN_RES_DIR}config/liquibase/changelog/${this.entityConfig.data.changelogDate}_added_${type}_${this.entityConfig.entityClass}.xml`;

        const files = {
            config: this.entityConfig.filename,
            ORM: `${jhipsterConstants.SERVER_MAIN_SRC_DIR + this.packageFolder}/domain/${this.entityConfig.entityClass}.java`,
